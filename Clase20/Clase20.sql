...
select count(*) cantidad from facturas;				-- 46
select 50*46 cantidad;								-- 2300
select count(*) cantidad from clientes,facturas;	-- 2300

-- Consulta del producto relacionado
select * from clientes, facturas where clientes.codigo=facturas.codigo_cliente;
-- uso del alieas en tablas
select * from clientes c, facturas f where c.codigo=f.codigo_cliente;

select count(*) cantidad from clientes c, facturas f where c.codigo=f.codigo_cliente; -- 46

-- Uso del Join
select * from clientes c join facturas f on c.codigo=f.codigo_cliente;

-- listar quienes (nombre,apellido) compraron en el día de hoy?
select c.nombre, c.apellido 
	from clientes c join facturas f on c.codigo=f.codigo_cliente
    where f.fecha=curdate();

-- TODO uso de left join y rigth join
