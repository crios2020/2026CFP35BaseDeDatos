...
select CONCAT(YEAR(fecha),'/',MONTH(fecha)) mes, ROUND(sum(monto),2) total 
	from facturas 
	GROUP BY YEAR(fecha),MONTH(fecha);
-- 3-	Informar total facturado (suma de monto) y cuantas facturas (count(*)) se realizaron en cada año.
select year(fecha) año, ROUND(sum(monto),2) total, count(*) cantidad
	from facturas
	GROUP BY year(fecha);
...
select sum(stock) cantidad from articulos where descripcion='Televisores';
-- 2- Informar cantidad de televisores segun marca.
select marca,sum(stock) cantidad from articulos where descripcion='Televisores' GROUP BY marca;
-- 3- Informar cual es el televisor más barato.
select * from articulos 
	where descripcion='Televisores' 
	and precio=(select min(precio) from articulos WHERE descripcion='Televisores');

-- 4- Aumentar un 15% los articulos de la marca 'LG'.
update articulos set precio=round(precio*1.15,2) where descripcion='Televisores' and marca='LG';
-- 5- Bajar un 4% los aires acondicionados.
update articulos set precio=ROUND(precio*0.96,2) where descripcion='Aire Acondicionado';
...
select marca,descripcion,sum(stock) cantidad from articulos group by marca, descripcion;
-- 8- Mostrar el producto mas caro de la marca 'LG'
select * from articulos 
	where marca='LG' 
	and precio=(select max(precio) from articulos where marca='LG');
